...
3. **Branch — is this a multi-session build?**
   ...
   Either way, **`/implement`** builds each issue by driving **`/tdd`** internally — one red-green slice at a time — then closes out with one independent **`/code-review`** of behavior, requirements, and repository standards before handing the tested work back to you to commit. Reach for **`/tdd`** on its own when you just want to build a concrete behaviour test-first without a full spec, and **`/code-review`** on its own whenever you want to review a branch or PR against a fixed point.
...
- **`/research`** — delegate source retrieval to a read-only researcher, then have the main agent verify and synthesize the result. It investigates **primary sources** and writes a repository note only when you explicitly request one. Research feeds the thinking; it doesn't replace it.
